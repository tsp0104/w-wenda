...
instance.interceptors.response.use(res => {
  ...
  if (err.response && err.response.status === 401) {
    // 拦截登陆的条状地址配置 （vue组件：this.$route.path）=== (router.currentRoute)
    ...
    try {
      // token失效
      // 此时使用instabnce将会有一些配置已经生效了。头部需要携带的是refresh_token
...
export default (url, method, data) => {
  return instance({
    ...
    [method.toLowerCase() === 'get' ? 'parse' : 'data']: data
  })
}
